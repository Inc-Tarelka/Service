...
func main() {
	...
	tgBase := os.Getenv("TELEGRAM_API_BASE_URL")
	proxySecret := os.Getenv("TELEGRAM_PROXY_SECRET")
	...
	log.Printf("[binder] TELEGRAM_API_BASE_URL=%s", tgBase)
	...
	for {
		...
		if err := pollOnce(client, botToken, apiBase, tgBase, proxySecret, &offset); err != nil {
...
func pollOnce(client *http.Client, botToken, apiBase, tgBase, proxySecret string, offset *int64) error {
	base := strings.TrimRight(tgBase, "/")
	url := base + "/bot" + botToken + "/getUpdates?timeout=10"
	if *offset > 0 {
	...
	if proxySecret != "" {
		req.Header.Set("X-Secret", proxySecret)
	}
